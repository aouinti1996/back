...
public class EventServiceImp implements IEventService {
	...
	private final CommentEventRepository commentRepository;
	private final NoteERepository noteERepository;
	private final LikeEventRepo lEventRepo;
	...
	/* NOTE EVENT */
	@Override
	public void noteEvent(Long idEvent, EventNote eventNote, Long idUser) {
	...
	/* REMOVE NOTE */
	@Override
	public void removeNote(Integer idNote, Long idEvent) {
	...
	/* COMMENT EVENT */
	@Override
	public void commentEvent(Long idEvent, Long idUser, String comment) {
	...
	/*USERS BY COMMENT*/
	@Override
	public List<User> getUsersByComment(Long idEvent){
		...
		for(int i=0; i<lu.size(); i++) {
			for(int j=1; j<lu.size(); j++) {
				if(lu.get(i).getUserId() == lu.get(j).getUserId()) {
					lu.remove(j);
				}
			}
		}
	...
	/*LIKE EVENT*/
	@Override
	public void likeEvent(Long idEvent, Long idUser) {
	...
	/*USERS BY LIKES*/
	@Override
	public List<User> getUsersByLikes(Long IdEvent){
		Event e = eventRepository.findById(IdEvent).get();
	...
	/*REMOVE LIKE*/
	@Override
	public void removeLikeEvent(Long idEvent, Long idUser) {
		Event e = eventRepository.findById(idEvent).get();
		List<LikeEvent> le = lEventRepo.findByEvent(e);
		for(LikeEvent l : le) {
			if(l.getUser().getUserId().equals(idUser)) {
	...
	/* JOIN EVENT */
	@Override
	public void joinEvent(Long idEvent, Long idUser) {
		Event e = eventRepository.findById(idEvent).get();
	...
	/*UNJOIN EVENT*/
	@Override
	public void unjoinEvent(Long idEvent, Long idUser) {
		Event e = eventRepository.findById(idUser).get();
	...
	public List<User> getUsersByCI(Long idUser) {
		...
		for(int i=0; i<lu.size(); i++) {
			if (lu.get(i).getUserId()==u.getUserId()) {
				lu.remove(lu.get(i));
			}
		}
		for(User user : lu) {
		...
		for(int i=0; i<luFinal.size(); i++) {
			for(int j=1; j<luFinal.size(); j++) {
				if(luFinal.get(i).getUserId() == luFinal.get(j).getUserId()) {
					luFinal.remove(j);
				}
			}
		}
	...
	@Override
	public void AddEventToFav(Long idEvent, Long idUser) {
		Event e = eventRepository.findById(idEvent).get();
		User u = userRepository.findById(idUser).get();
